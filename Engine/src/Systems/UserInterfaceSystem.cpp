...
namespace ECS {
    ...
    void ImageSystem::Start(Coordinator &coordinator, AssetManager &assetManager)
    {
        for (auto const &entity : entities) {
            auto &image = coordinator.GetComponent<Image>(entity);

            auto assetOpt = assetManager.getAsset(image.fileName);
            if (assetOpt) image.texture = assetOpt.value()->GetData<Texture2D>();
            else std::cerr << "Texture " << image.fileName << " not found" << std::endl;

            image.texture.width *= image.scale.x;
            image.texture.height *= image.scale.y;
            image.set = true;
        }
    }

    void ImageSystem::Update(Coordinator &coordinator, AssetManager &assetManager)
    {
        for (auto const &entity : entities) {
            auto &image = coordinator.GetComponent<Image>(entity);
            ...
            if (image.draw)
                DrawTextureEx(image.texture, image.position, image.rotation, 1, image.tint);
        }
    }
}
